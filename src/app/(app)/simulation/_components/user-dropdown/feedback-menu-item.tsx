...
import { Button, Form } from "~/components/ui/core";
import { toast } from "sonner";
import { api } from "~/lib/trpc/react";
import { Controller, useForm } from "react-hook-form";
import { FeedbackType, feedbackZod } from "prisma/zod-utils";
...
export default function FeedbackMenuItem({ closeDropdownMenu }: { closeDropdownMenu: () => void }) {
    const feedbackForm = useForm<FeedbackType>({
        resolver: zodResolver(feedbackZod),
    })
    const { register, formState: { isValid } } = feedbackForm
    const mutation = api.user.submitFeedback.useMutation({
        onSuccess: () => {
            toast.success("Thank you, feedback submitted");
            closeDropdownMenu();
        },
        onError: () => {
            toast.error("Could not send feedback email. Please try again");
        }
    });
    ...
    return (
        ...
        >
            <div className="p-5">
                <p className="mb-1 opacity-50">Feedback</p>
                <p className="flex w-full py-2 text-sm font-medium text-foreground-emphasis">
                    Share your comments here:
                </p>
                <textarea
                    rows={3}
                    className={cn(
                        "my-1 block w-full rounded-sm border border-gray-300 py-2 px-3 text-sm transition-colors placeholder:text-gray-400 [&:not(:focus)]:hover:border-gray-400",
                ...
                <div className="my-3 flex justify-end">
                    {Object.entries(RATING_ICONS).map(([rating, icon], index) => (
                        <Controller key={index} name="rating" render={({ field }) => (
                            <button
                                {...field}
                                type='button'
                                className={cn(
                                    "m-1 items-center justify-center p-1.5 grayscale hover:opacity-100 hover:grayscale-0",
                                    field.value === rating ? "grayscale-0" : "opacity-50"
                                )}
